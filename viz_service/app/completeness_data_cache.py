...
from .HttpCall import get_all_units, get_completeness_data
import datetime
...
def gen_cache():
    ...
    for unit_id in unit_ids:
        for date_range in date_rangers:
            viz_result = get_completeness_data(unit_id, date_range)
            print('unit_id:', unit_id, 'date_range:', date_range)
            # if viz_result != 'No data' and viz_result != {'output_completeness': {}, 'report_rate': {}}:
            #     b = ''
            key = 'completeness_report_' + unit_id + '_' + date_range
...
class completeness_data_cache(Resource):

    def post(self):
        return {'data': start_completeness_cache()}
