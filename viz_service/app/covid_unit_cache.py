...
from .HttpCall import get_covid_unit_data, get_all_units
import datetime
...
def gen_cache():
    ...
    for unit_id in unit_ids:
        for date_range in date_rangers:
            viz_result = get_covid_unit_data(unit_id, date_range)
            key = f'covid_unit_stats_{unit_id}_{date_range}'
            print(key)
...
class covid_unit_data_cache(Resource):

    def post(self):
        return {'data': start_covid_unit_cache()}
